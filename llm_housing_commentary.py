...
def request_commentary(json_data: str, model: str, max_tokens: int = 500) -> str:
    ...
    system_prompt = (
        ...
        "1) Marktdaten (historische Zinssätze gruppiert nach Fixlaufzeit und Laufzeit),"
        "2) Wettbewerbsangebote (tatsächliche Angebote von Konkurrenzbanken). "
        "\n\n"
        "Deine Analyse soll folgende Punkte abdecken: "
        "• Aktuelle Marktdynamik: Ich will eine Aufzählung der Veränderungen der Preise von allen Fixlaufzeiten der 25J Laufzeit als Liste zwischen dem letzten und dem vorletzten Scraping-run. "
        "WICHTIG: Verwende die vorkalkulierten Werte aus market_data_by_fixierung_laufzeit: "
        "Für jede Fixlaufzeit (0J, 5J, 10J, 15J, 20J, 25J) mit laufzeit_jahre=25: "
        ...
        "- Formatiere als: 'X Jahre: latest% (+change_in_bps Basispunkte)' "
        "Beispiel: latest=3.13, change_week=0.03"
        "Ausgabe: '5 Jahre: 3.13 (+3 Basispunkte)'. "
        "Für 0J Fixlaufzeit schreibe 'variabel' statt '0 Jahre'. "
        "Ich will keine prozentualen Veränderungen, nur die Basispunkte als ganze Zahlen."
        "• Konkurrenzanalyse: Gib eine Übersicht der 3 neuesten Konkurrenzangebote. Beziehe Dich nur auf die aktuellsten Angebote (such nach den 3 neuesten angebotsdatum in competitor_offers). Nimm die drei konkreten Angebote, nenne den Zinssatz, Fixlaufzeit und Laufzeit, sowie die Bank."
        "\n\n"
        "Stil: Präzise, faktenbasiert, für Bankprofis. Verwende Fachbegriffe korrekt. "
        "Wenn du über Zinssätze berichtest, sage immer dazu, welche Fixlaufzeit und Laufzeit gemeint sind. "
        "Die Variante 0J, oder 0-jährige Fixlaufzeit nennt man variabel. Bitte nenne sie überall auch so."
    ...
    user_prompt = (
        ...
        "JSON-Daten:\n"
        f"{json_data}\n\n"
        "Erstelle eine prägnante, professionelle Marktanalyse für Bankprofis. Ziehe am Ende kein Fazit und kein Urteil"
    )
