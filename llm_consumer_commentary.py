#!/usr/bin/env python3
"""
Generate a short German interpretation of the consumer loan comparison report
...
def request_commentary(json_data: str, model: str, max_tokens: int = 500) -> str:
    ...
    system_prompt = (
        ...
        "Die Daten enthalten eine Zusammenfassung (summary), eine vollständige Zeitreihe (time_series_data) "
        "und eine voraggregierte Struktur per_bank_changes. "
        "\n\n"
        "Deine Analyse soll folgende Punkte abdecken: "
        "• Aktuelle Marktdynamik: Ich will eine Aufzählung der Veränderungen der Preise von allen Banken als Liste "
        "zwischen dem letzten und dem vorletzten Scraping‑Run. "
        "WICHTIG: Verwende AUSSCHLIESSLICH die voraggregierten Werte aus per_bank_changes (nicht selbst aus time_series_data berechnen): "
        "Für jede Bank (bank_name) in per_bank_changes: "
        "- Nimm latest.effektiver_jahreszins als aktuellen Wert (z.B. 6.43) "
        "- Nimm change.delta_bps als Veränderung in Basispunkten (z.B. 15) "
        "- Formatiere als: 'Bankname: aktueller% (+change_in_bps Basispunkte)' "
        "Beispiel: latest=6.43, delta_bps=15"
        "Ausgabe: 'Bank XYZ: 6.43 (+15 Basispunkte)'. "
        "Nur Banken auflisten, für die per_bank_changes vorhanden ist. "
        "Ich will keine prozentualen Veränderungen, nur die Basispunkte als ganze Zahlen. "
        "• Zusammenfassung: Gib eine kurze Zusammenfassung der aktuellen Zinssätze (höchster und niedrigster), "
        "basierend auf summary.statistics.effektiver_jahreszins (min/max), aber wiederhole nicht alle Einzelwerte."
        "\n\n"
...
def embed_commentary(original_html: str, commentary: str) -> str:
    ...
    section_html = (
        ...
        "  <h2 style=\"margin-top: 0; color: #1f77b4;\">AI Analyse & Kommentar – Konsumkredit (beta)</h2>\n"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Erzeugt einen LLM-Kommentar für den Konsumkredit-HTML-Bericht."
    )
    parser.add_argument(
        "--input",
        type=Path,
